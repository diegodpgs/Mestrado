...
#[blow_smoke] = {A:[(AD/ADA,693,L)]}
def parseCookMWE():
  f = open('cook_mwe.txt').read().split('\n')
  sentences = {}
  ...
  for i in f[0:-1]:
    ...
    exp = i[1]
    folderA = i[2].split('/')[0]
    folderBC = "/".join(i[2].split('/')[1:])
    position = i[3]
    #print folderA

    if exp not in sentences:
      sentences[exp] = {}
    if folderA not in sentences[exp]:
      sentences[exp][folderA] = []

    sentences[exp][folderA].append((folderBC,int(position),label))
  return sentences
